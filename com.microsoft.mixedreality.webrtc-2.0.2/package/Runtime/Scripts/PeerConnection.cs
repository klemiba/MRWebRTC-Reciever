    ...
    public class PeerConnection : WorkQueue, ISerializationCallbackReceiver
    {
        ...
        public bool StartConnection()
        {
            ...
            foreach (var mediaLine in _mediaLines) {
                Debug.Log("Processing media line: " + index + "/" + _mediaLines.Count);
                ...
                if (tr != null)
                {
                    ...
                    Debug.Log("Creating transciever: " + settings.ToString());
        ...
        public async Task HandleConnectionMessageAsync(SdpMessage message) {
            ...
            foreach (var tr in transceivers) {
                foreach (var stream in tr.StreamIDs) {
                    Debug.Log("This is the stream ID from remote: " + stream);
                }
            }
            
            // Debug.Log("Num matching: " + numMatching);
            // Debug.Log("Num associated: " + numAssociatedTransceivers);
            // Debug.Log("Num medlines: " + _mediaLines.Count);
            ...
            if (message.Type == SdpMessageType.Offer) {
                ...
                {
                    // Debug.Log("Transceiver direction: " + transceivers[i].DesiredDirection);
                    // Debug.Log("Checkstate: " + Transceiver.DirectionFromSendRecv(true, false) );
                    if (transceivers[i].DesiredDirection == Transceiver.DirectionFromSendRecv(true, false)) {
                        ii++;
                        continue;
                    }

                    
                    Debug.Log("Pairing: " + i + " with " + (i - ii));
                    var tr = transceivers[i];
                    var mediaLine = _mediaLines[i - ii];

                    // Debug.Log("Trensceiver and medialine set.");
                    /*
                    ...
                    if (mediaLine.Transceiver == null) {
                        ...
                        try {
                            ...
                            Debug.LogError("Exception encountered when pairing: " + ex);
                        }
                        
                        Debug.Log("Pairing transceiver.");
                    } else {
            ...
            else if (message.Type == SdpMessageType.Answer)
            {
                ...
                for (int i = 0; i < numMatching; ++i) {
                    ...
                    mediaLine.UpdateAfterSdpReceived();
                    Debug.Log("Media and transceiver match!");
                }
        ...
        private void OnEnable() {
            ...
            Debug.Log("Initializing Peer Connection");
            InitializeAsync();
        }
